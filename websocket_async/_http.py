...
async def connect(url, options, proxy, socket):
    ...
    try:
        ...
        if need_tunnel:
            sock = await _tunnel(sock, hostname, port, auth)
...
async def _tunnel(sock, host, port, auth):
    ...
    await send(sock, connect_header)

    try:
        status, resp_headers, status_message = await read_headers(sock)
